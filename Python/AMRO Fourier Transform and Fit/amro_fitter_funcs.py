...
class FitAMRO:
	...
	def SineBuilder(self, rads, amps : np.array, freqs : np.array, phases: np.array, mean:float|int):


		summation = np.sum(amps[:, None] * np.sin(freqs[:, None] * rads + phases[:, None]), axis=0)
	...
	def FitAMROData(self, f_list:list, ACT:str, H:int|float, T:int|float)->lm.minimizer.MinimizerResult:
		'''
		...
		q = 'ACT_str == "{}" & H == {} & T== {} & `freqs (cycles/rot)` in @f_list'.format(ACT, H, T)
	...
	def _initialize_parameters(self, freqs_list:list, guesses:pd.DataFrame, mean:float)->lm.Parameters:
	...
	def _add_freq_guess(self, frequency:int, params:lm.Parameters, mean_val:float, guesses_df:pd.DataFrame)->None:
		temp_df = guesses_df.query('`freqs (cycles/rot)` == {}'.format(frequency))
		params.add('amp'+str(frequency),
		...
		params.add('freq'+str(frequency),
					value = temp_df['freqs (cycles/rot)'].values[0],
					vary=False)

		params.add('phase'+str(frequency),
	...
	def FitACTExperiment(self, label, f_rank_min, f_ratio_min_ratio, f_max, show_fig = False): 
